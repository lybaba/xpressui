...
| `demos/remote-resume-flow.html` | Remote save/resume, share code, combined ops inspection | `createResumeTokenAsync`, `restoreFromResumeTokenAsync`, `claimResumeShareCodeDetail`, `restoreFromShareCodeDetailAsync`, `getResumeShareCodeClaimPresentation`, `createFormOpsPanel`, `createLocalFormAdmin` | `form-ui:resume-token-created`, `form-ui:resume-share-code-claim-state`, `form-ui:resume-share-code-restore-state`, `form-ui:provider-contract-warning` |
