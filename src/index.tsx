...
function renderPostUI(props: RenderPostUIProps) {
    ...
    const imagesBaseUrl = `${baseUrl}postui/images`;
...
function render(props: PostUIProps) {
    const {
        postConfig,
        baseUrl = '',
        template = '',
...
class PostUIWebComponent extends HTMLElement {
    connectedCallback() {
        ...
        getConfig(`postui/${postName}-config.json`, `postui/${postName}-assets.json`).then((data) => {
