...
function renderField(field: TFieldConfig, sectionName: string): string {
  ...
  const fileSelectionMarkup = isFileFieldType(field.type)
    ? field.type === DOCUMENT_SCAN_TYPE
      ? `<div class="mt-2 rounded border border-dashed border-base-300 p-3 transition-colors" id="${escapeHtml(field.name)}_selection" data-file-drop-zone="${escapeHtml(field.name)}"><div class="flex items-center justify-between gap-2"><div class="text-sm font-medium" data-upload-selection-title="${escapeHtml(field.name)}">Awaiting document scan</div><span class="rounded-full border border-base-300 px-2 py-0.5 text-xs" data-upload-selection-kind="${escapeHtml(field.name)}">Document scan</span></div><div class="mt-2 text-xs opacity-70" data-upload-selection-message="${escapeHtml(field.name)}">Capture or upload the front and back of your document.</div><div class="mt-3 grid gap-2" data-upload-selection-body="${escapeHtml(field.name)}"><div class="mb-3 flex flex-wrap gap-2" data-document-scan-controls="${escapeHtml(field.name)}"><button type="button" class="btn btn-xs btn-outline btn-primary" data-document-scan-slot="0">Capture Front</button>${field.documentScanMode === 'single' ? '' : '<button type="button" class="btn btn-xs btn-outline" data-document-scan-slot="1">Capture Back</button>'}</div><div class="grid gap-3 md:grid-cols-2" data-document-scan-grid="${escapeHtml(field.name)}"><div class="rounded border border-base-300 p-3" data-document-scan-slot-card="${escapeHtml(field.name)}:0"><div class="mb-2 text-xs font-medium uppercase opacity-70">Front</div><div class="flex h-28 w-full items-center justify-center overflow-hidden rounded border border-base-300 bg-base-200" style="aspect-ratio:1.586 / 1" data-document-scan-preview="${escapeHtml(field.name)}:0"><div class="px-2 text-center text-xs opacity-70" data-document-scan-placeholder="${escapeHtml(field.name)}:0">No scan yet</div></div><div class="mt-2 text-xs" data-document-scan-file-name="${escapeHtml(field.name)}:0"></div><div class="mt-2 text-[11px] opacity-80" data-document-scan-ocr="${escapeHtml(field.name)}:0">${field.enableDocumentOcr ? 'OCR: waiting for extraction' : ''}</div><div class="mt-1 text-[11px] font-medium" data-document-scan-mrz="${escapeHtml(field.name)}:0">${field.documentMrzTargetField || field.requireValidDocumentMrz ? 'MRZ: waiting for detection' : ''}</div></div>${field.documentScanMode === 'single' ? '' : `<div class="rounded border border-base-300 p-3" data-document-scan-slot-card="${escapeHtml(field.name)}:1"><div class="mb-2 text-xs font-medium uppercase opacity-70">Back</div><div class="flex h-28 w-full items-center justify-center overflow-hidden rounded border border-base-300 bg-base-200" style="aspect-ratio:1.586 / 1" data-document-scan-preview="${escapeHtml(field.name)}:1"><div class="px-2 text-center text-xs opacity-70" data-document-scan-placeholder="${escapeHtml(field.name)}:1">No scan yet</div></div><div class="mt-2 text-xs" data-document-scan-file-name="${escapeHtml(field.name)}:1"></div><div class="mt-2 text-[11px] opacity-80" data-document-scan-ocr="${escapeHtml(field.name)}:1">${field.enableDocumentOcr ? 'OCR: waiting for extraction' : ''}</div><div class="mt-1 text-[11px] font-medium" data-document-scan-mrz="${escapeHtml(field.name)}:1">${field.documentMrzTargetField || field.requireValidDocumentMrz ? 'MRZ: waiting for detection' : ''}</div></div>`}</div><div class="mt-2 text-xs opacity-70" data-document-scan-helper="${escapeHtml(field.name)}">Scans are center-cropped to an ID card frame before submit.</div></div></div>`
  ...
  if (field.type === PRODUCT_LIST_TYPE) {
    ...
    <div class="mt-2 rounded border border-base-300 p-3" style="padding:12px;" id="${escapeHtml(field.name)}_selection" data-product-list-zone="${escapeHtml(field.name)}"></div>
    ${helpText}
    <div class="label"><span class="label-text-alt" id="${escapeHtml(field.name)}_error"></span></div>
</div>`;
  }
